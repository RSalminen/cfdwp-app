...
const Selection = ({selectedItem, onChangeFn, thisItem, allItems, light, fullWidth=false}: {thisItem:string | null, selectedItem:string | null, onChangeFn:Function, allItems:string[], light?:boolean, fullWidth?:boolean }) => {
    ...
    if (light) return (
        <div className={`${fullWidth && "w-full"} relative text-black bg-white`}>
            <div ref={ref}>
                <div className="py-2 px-2 flex items-center text-[15px] md:cursor-pointer border border-gray-400 w-full rounded-t-sm" onClick={() => setIsComponentVisible(!isComponentVisible)}>
                ...
                <div className={`absolute z-[2] w-full border-b border-x border-gray-400 shadow-lg shadow-gray-800 bg-white ${isComponentVisible ? "visible" : "invisible h-0"}`}>
