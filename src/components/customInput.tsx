...
const CustomInput = ({labelText, onChange, currentValue, showPassword}:{labelText:string, onChange:Function, currentValue:string, showPassword?:boolean}) => {
    ...
    return (
        <div className="flex flex-col border-b-2 border-emerald-700 bg-gray-200 pb-0.5 rounded-t-sm w-full">
            ...
            <input className="text-[13px] focus:outline-none bg-gray-200 px-1.5" type={showPassword !== false ? "text" : "password"} onKeyPress={stopPropagation} onKeyUp={stopPropagation} onKeyDown={stopPropagation} onChange={valueChanged} value={currentValue} />
        </div>
    )
}
