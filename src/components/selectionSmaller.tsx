...
const SelectionSmaller = ({selectedItem, onChangeFn, thisItem, allItems, light, fullWidth=false}: {thisItem:string | null, selectedItem:string | null, onChangeFn:Function, allItems:string[], light?:boolean, fullWidth?:boolean }) => {
    ...
    if (light) return (
        ...
        <div className={`min-w-[134px] w-fit ${fullWidth && "w-full"} relative text-white bg-black`}>
            <div ref={ref}>
                <div className="py-1.5 pr-1 pl-2 flex items-center text-[13px] md:cursor-pointer border w-full rounded-t-sm" onClick={() => setIsComponentVisible(!isComponentVisible)}>
                ...
                <div className={`absolute z-[2] w-full border-b border-x border-emerald-100 shadow-md shadow-gray-800 bg-black ${isComponentVisible ? "visible" : "invisible h-0 overflow-y-hidden"}`}>
                    ...
                    <div className="relative">
                        <div className="min-h-fit max-h-[300px] overflow-y-auto pb-[1px]">                                
                            {allItems.map((item:string) => 
                                <div key={item} onClick={() => changeSelection(item)} className={`my-[-1px] text-[12px] overflow-hidden text-ellipsis whitespace-nowrap px-2.5 py-1 w-full ${selectedItem !== item && "md:hover:bg-gray-400"} md:cursor-pointer ${selectedItem === item && "bg-gray-800"}`}>{item}</div>
                            )}
                        </div>
                    </div>
                </div>            
